pipeline{
    ...
    stages{
        ...
        stage("Trivy Scan"){
            steps{
                script{
                    sh ("docker run -v /var/run/docker.sock:/var/run/docker.sock aquasec/trivy image brahim02/projet-devsecops:latest --no-progress --exit-code 0 --severity HIGH,CRITICAL --format table")
                }
            }
        }
